...
class RequestAppraisal(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get('default_appraisal_id'):
            appraisal = self.env['appraisal.appraisal'].browse(self.env.context['default_appraisal_id'])
            employee = appraisal.employee_id
            managers = appraisal.manager_ids
            if self.env.user.employee_id in managers:
                template = self.env.ref('aspl_hr_appraisal.mail_template_appraisal_request', raise_if_not_found=False)
                recipients = self._get_recipients(employee)
            elif employee.user_id == self.env.user:
                template = self.env.ref('aspl_hr_appraisal.mail_template_appraisal_request_from_employee',
    ...
    appraisal_id = fields.Many2one('appraisal.appraisal', required=True)
    ...
    def _compute_subject(self):
        for wizard in self.filtered('employee_id'):
            if wizard.template_id:
                wizard.subject = \
                self.sudo()._render_template(wizard.template_id.subject, 'res.users', self.env.user.ids,
                                             post_process=True)[self.env.user.id]

    @api.depends('template_id', 'recipient_ids')
    def _compute_body(self):
        for wizard in self:
            user_body = wizard.user_body
            user_body = user_body if not is_html_empty(
                html_sanitize(user_body, strip_style=True, strip_classes=True)) else False
            if wizard.template_id:
                ctx = {
                ...
                wizard.body = self.with_context(ctx).sudo()._render_template(wizard.template_id.body_html, 'res.users',
                                                                             self.env.user.ids, post_process=False,
                                                                             engine='qweb')[self.env.user.id]
    ...
    def _compute_render_model(self):
        self.render_model = 'appraisal.appraisal'
    ...
    def action_invite(self):
        ...
        ctx = {
            'url': '/mail/view?model=%s&res_id=%s' % ('appraisal.appraisal', appraisal.id),
        }
